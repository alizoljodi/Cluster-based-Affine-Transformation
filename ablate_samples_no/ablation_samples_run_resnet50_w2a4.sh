#!/bin/bash
#SBATCH -J R50_W2A4_SAMPLES_ABLATION
#SBATCH -c 8
#SBATCH --mem=128G
...
echo "Starting ResNet50 W2A4 samples ablation experiment at $(date)"
...
for num_samples in "${samples_values[@]}"; do
    ...
    python ../main_imagenet.py --data_path /home/alz07xz/imagenet --arch resnet50 \
        --n_bits_w 2 --n_bits_a 4 --weight 0.01 --T 4.0 --lamb_c 0.02 --seed 1000 \
        --alpha 0.5 --num_clusters 64 --pca_dim 50 --num_samples $num_samples
    sleep 0.5
done

echo "Completed ResNet50 W2A4 samples ablation experiment at $(date)"
