#!/bin/bash
#SBATCH -J MNAS_W4A2_SAMPLES_ABLATION
#SBATCH -c 8
#SBATCH --mem=128G
...
echo "Starting MnasNet W4A2 samples ablation experiment at $(date)"
...
for num_samples in "${samples_values[@]}"; do
    ...
    python ../main_imagenet.py --data_path /home/alz07xz/imagenet --arch mnasnet \
        --n_bits_w 4 --n_bits_a 2 --weight 0.2 --T 1.0 --lamb_c 0.001 --seed 1000 \
        --alpha 0.5 --num_clusters 64 --pca_dim 50 --num_samples $num_samples
    sleep 0.5
done

echo "Completed MnasNet W4A2 samples ablation experiment at $(date)"
