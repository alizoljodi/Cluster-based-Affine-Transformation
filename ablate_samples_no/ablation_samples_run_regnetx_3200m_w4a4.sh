#!/bin/bash
#SBATCH -J REG3200_W4A4_SAMPLES_ABLATION
#SBATCH -c 8
#SBATCH --mem=128G
...
echo "Starting RegNetX-3200M W4A4 samples ablation experiment at $(date)"
...
for num_samples in "${samples_values[@]}"; do
    ...
    python ../main_imagenet.py --data_path /home/alz07xz/imagenet --arch regnetx_3200m \
        --n_bits_w 4 --n_bits_a 4 --weight 0.01 --T 4.0 --lamb_c 0.01 --seed 1000 \
        --alpha 0.5 --num_clusters 64 --pca_dim 50 --num_samples $num_samples
    sleep 0.5
done

echo "Completed RegNetX-3200M W4A4 samples ablation experiment at $(date)"
